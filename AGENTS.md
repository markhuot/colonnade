...
- Use `Logger(subsystem: "ai.opencode.mac", category: "your-category")` from `OSLog`.
- Prefer `.notice` for temporary diagnostics you want to inspect with `log show`.
- To inspect logs from the running app:

```bash
/usr/bin/log show --last 10m --style compact --info --predicate 'process == "OpenCodeMac" && subsystem == "ai.opencode.mac" && category == "your-category"'
```
